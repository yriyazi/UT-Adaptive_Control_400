...
for i = Nv+1:N
    ...
    if i<20
        ...
        [teta,P_cont]    =RLS(teta,phi,P_cont,Nv,y(i));
        Rst = teta(1:Nv/2)'         ;   R(1:Nv/2,i)=teta(1:Nv/2)    ;
        Sst = teta(Nv/2 +1:Nv)'     ;   S(1:Nv/2,i)=teta(Nv/2 +1:Nv);
        t0=sum(A_m)/sum(Best)       ;
...
RLS_Convergence(2,R,'r',2,S,'s',t,Titlework,1,plot_counter)
        plot_counter=plot_counter+1;
